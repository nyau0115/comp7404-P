...
class DMN:
    ...
    def __init__(self, babi_train_raw, babi_test_raw, word2vec, word_vector_size, sent_vector_size, 
        ...
        self.W_pe = helper_utils.normal_param(std=0.1, shape=(self.vocab_size, self.dim))

        #biGRU input fusion weights
        self.W_inp_res_in_fwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.sent_vector_size))
        self.W_inp_res_hid_fwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_inp_res_fwd = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        
        self.W_inp_upd_in_fwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.sent_vector_size))
        self.W_inp_upd_hid_fwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_inp_upd_fwd = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        
        self.W_inp_hid_in_fwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.sent_vector_size))
        self.W_inp_hid_hid_fwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_inp_hid_fwd = helper_utils.constant_param(value=0.0, shape=(self.dim,))

        self.W_inp_res_in_bwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.sent_vector_size))
        self.W_inp_res_hid_bwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_inp_res_bwd = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        
        self.W_inp_upd_in_bwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.sent_vector_size))
        self.W_inp_upd_hid_bwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_inp_upd_bwd = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        
        self.W_inp_hid_in_bwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.sent_vector_size))
        self.W_inp_hid_hid_bwd = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_inp_hid_bwd = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        ...
        self.W_mem_res_in = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.W_mem_res_hid = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_mem_res = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        
        self.W_mem_upd_in = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.W_mem_upd_hid = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_mem_upd = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        
        self.W_mem_hid_in = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.W_mem_hid_hid = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_mem_hid = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        
        #self.W_b = nn_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        #self.W_1 = nn_utils.normal_param(std=0.1, shape=(self.dim, 7 * self.dim + 0))
        self.W_1 = helper_utils.normal_param(std=0.1, shape=(self.dim, 4 * self.dim + 0))
        self.W_2 = helper_utils.normal_param(std=0.1, shape=(1, self.dim))
        self.b_1 = helper_utils.constant_param(value=0.0, shape=(self.dim,))
        self.b_2 = helper_utils.constant_param(value=0.0, shape=(1,))
        ...
        self.W_a = helper_utils.normal_param(std=0.1, shape=(self.vocab_size, self.dim))
        
        if self.answer_module == 'feedforward':
            self.prediction = helper_utils.softmax(T.dot(self.W_a, last_mem))
        
        elif self.answer_module == 'recurrent':
            self.W_ans_res_in = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim + self.vocab_size))
            self.W_ans_res_hid = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
            self.b_ans_res = helper_utils.constant_param(value=0.0, shape=(self.dim,))
            
            self.W_ans_upd_in = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim + self.vocab_size))
            self.W_ans_upd_hid = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
            self.b_ans_upd = helper_utils.constant_param(value=0.0, shape=(self.dim,))
            
            self.W_ans_hid_in = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim + self.vocab_size))
            self.W_ans_hid_hid = helper_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
            self.b_ans_hid = helper_utils.constant_param(value=0.0, shape=(self.dim,))
            ...
            def answer_step(prev_a, prev_y):
                ...
                y = helper_utils.softmax(T.dot(self.W_a, a))
                return [a, y]
        ...
        if self.l2 > 0:
            self.loss_l2 = self.l2 * helper_utils.l2_reg(self.params)
        else:
            self.loss_l2 = 0
    ...
    def _process_input(self, data_raw):
        ...
        for x in data_raw:
            ...
            for i in range(len(inp)):            
                ...
                inp_i = [babi_utils.process_word(word = inp[i][w],
            ...
            q_vector = [babi_utils.process_word(word = q[w],
                                                word2vec = self.word2vec,
                                                vocab = self.vocab,
                                                ivocab = self.ivocab,
                                                word_vector_size = self.word_vector_size,
            ...
            answers.append(babi_utils.process_word(word = x["A"],
                                                   word2vec = self.word2vec,
                                                   vocab = self.vocab,
                                                   ivocab = self.ivocab,
                                                   word_vector_size = self.word_vector_size,
    ...
    def step(self, batch_index, mode):
        ...
        param_norm = np.max([babi_utils.get_norm(x.get_value()) for x in self.params])
